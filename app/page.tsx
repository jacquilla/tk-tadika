...
export default function AppTK() {
  ...
  // State untuk perbaikan props EditLogModal
  ...
  // Menangani pengiriman pesan massal dari BroadcastModal
  const handleKirimSiaran = async () => {
    ...
    for (const m of muridSemua) {
      ...
      if (tipeSiaran === "spp") {
        pesan = pesan.replace(
          /\[Nominal\]/g,
          `Rp ${((m as any).nominal_spp || 350000).toLocaleString("id-ID")}`,
        );
      }

      try {
        await kirimWA(m.nomor_hp_ortu, pesan);
        sukses++;
      } catch (err) {
    ...
    <>
      ...
      {tampilan === "kelas" && (
        <KelasScreen
          namaGuru={namaGuru}
          jumlahMawar={
            dataSemuaMurid.filter((m) => m.kelas.toLowerCase() === "mawar")
              .length
          }
          jumlahMelati={
            dataSemuaMurid.filter((m) => m.kelas.toLowerCase() === "melati")
              .length
          }
          onPilihKelas={(kelas) => {
            setKelasAktif(kelas);
      ...
      {tampilan === "dashboard" && (
        <div className="min-h-screen bg-gradient-to-b from-indigo-50 via-white to-purple-50 pb-32 fade-in">
          {/* Sinkronisasi Props DashboardHeader Terbaru */}
          <DashboardHeader
      ...
      {/* Sinkronisasi Props BroadcastModal Terbaru & Penambahan Logika Pengiriman */}
      <BroadcastModal
